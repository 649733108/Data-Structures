...
public class BST1<E extends Comparable<E>> {
	...
	private void preOrder(Node node) {
		...
		if (node != null) {
			System.out.println("node: " + node.e + "  Size: " + node.size + "  Depth: " + node.depth);
			preOrder(node.left);
			preOrder(node.right);
		}
	}
	...
	private void generateDepth(Node node, int depth){
		if (node!=null){
			node.depth = depth;
			generateDepth(node.left,depth+1);
			generateDepth(node.right,depth+1);
		}
	}
	/**
	 * 删除BST最小值递归算法
	...
	private Node removeMin(Node node) {
		if (node.left==null){
			...
			node.right = null;
			generateDepth(rightNode,1);
			return rightNode;
		}
		else {
			node.left = removeMin(node.left);
			node.size--;
//			generateDepth(node,1);
			return node;
		}
	}
